...
var NoiseWorley2DFrag = require('../../../renderer/webgl/shaders/NoiseWorley2D-frag');
 ...
 *
 * Behind the scenes, a NoiseCell2D is a {@see Phaser.GameObjects.Shader}
 ...
 */
var NoiseCell2D = new Class({
    Extends: Shader,

    initialize: function NoiseCell2D (scene, config, x, y, width, height)
    {
        if (!config) { config = {}; }

        var shaderConfig = {
            name: 'noiseCell2D',
            fragmentSource: NoiseWorley2DFrag,
            shaderAdditions: [
                {
                    name: 'MODE_DISTANCE',
                    tags: [ 'MODE' ],
     ...
     *
     * @method Phaser.GameObjects.NoiseCell2D#_setupUniforms
     * @private
     * @since 4.0.0
    ...
    _setupUniforms: function (setUniform)
    {
        ...
        setUniform('uSeedX', this.noiseSeed.slice(0, 2));
        setUniform('uSeedY', this.noiseSeed.slice(2, 4));
     ...
     *
     * @method Phaser.GameObjects.NoiseCell2D#_updateShaderConfig
     * @private
     * @since 4.0.0
     * @param {Phaser.Renderer.WebGL.DrawingContext} drawingContext - A reference to the current drawing context.
