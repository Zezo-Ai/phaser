...
Manages a list of render step functions for controlling the render pipeline.
...
### Filters (v4, WebGL only)

Post-processing filter system. Must call `enableFilters()` before use.
...
| `filtersFocusContext` | boolean | Focus on rendering context instead of object bounds. Default: `false` |
| `filtersForceComposite` | boolean | Always draw to framebuffer even without filters. Default: `false` |
| `maxFilterSize` | Vector2 | Max filter texture dimensions. |
...
| `focusFilters()` | method | Focus filter camera on the Game Object. Auto-called if `filtersAutoFocus` is true. Returns `this`. |
| `focusFiltersOverride(x?, y?, width?, height?)` | method | Manually override filter camera focus. Disables `filtersAutoFocus`. Returns `this`. |
| `setFiltersAutoFocus(value)` | method | Set `filtersAutoFocus`. Returns `this`. |
| `setFiltersFocusContext(value)` | method | Set `filtersFocusContext`. Returns `this`. |
| `setFiltersForceComposite(value)` | method | Set `filtersForceComposite`. Returns `this`. |
| `setRenderFilters(value)` | method | Set `renderFilters`. Returns `this`. |
| `setFilterSize(width, height)` | method | Set filter texture size. Returns `this`. |

### FilterList (v4, WebGL only)

Accessed via `gameObject.filters.internal` or `gameObject.filters.external`. Provides methods to add filter effects.

Key add methods (all return the created filter controller):

| Method | Description |
|---|---|
| `addBarrel(amount)` | Barrel distortion |
| `addBlend(texture, blendMode, amount, color)` | Blend with a texture |
| `addBlocky(config)` | Blocky pixelation effect |
| `addBlur(quality, x, y, strength, color, steps)` | Gaussian blur |
| `addBokeh(radius, amount, contrast)` | Bokeh depth-of-field |
| `addColorMatrix()` | Color matrix manipulation |
| `addDisplacement(texture, x, y)` | Displacement map |
| `addGlow(color, outerStrength, innerStrength, scale, knockout, quality, distance)` | Glow/outline |
| `addGradientMap(config)` | Gradient color mapping |
| `addKey(config)` | Chroma key (green screen) |
| `addMask(mask, invert, viewCamera, viewTransform, scaleFactor)` | Mask filter (WebGL masking) |
| `addPixelate(amount)` | Pixelation |
| `addSampler(callback, region)` | Custom sampling |
| `addShadow(x, y, decay, power, color, samples, intensity)` | Drop shadow |
| `addThreshold(edge1, edge2, invert)` | Threshold effect |
| `addTiltShift(radius, amount, contrast, blurX, blurY, strength)` | Tilt-shift |
...
Internal filters apply in object-local space (rotate with the object). External filters apply in screen space.
